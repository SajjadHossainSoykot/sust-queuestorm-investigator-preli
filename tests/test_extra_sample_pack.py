...
EXTRA_PACK_PATH = Path(__file__).resolve().parents[1] / "samples" / "extra_sample_cases.json"

def load_cases():
    with open(EXTRA_PACK_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)
    return data["test_cases"]

@pytest.mark.parametrize("case", load_cases(), ids=lambda c: c["id"])
def test_extra_case(case):
    input_data = case["input"]
    expected = case["expected_key_outputs"]
